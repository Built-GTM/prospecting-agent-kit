...
1. **Build their onboarding binder (context pack).** Follow `skills/build-context-pack/SKILL.md`. Read their website, draft the six files into a new `my-pack/` folder, and mark every field as found with a source link, or inferred. Then ask them the five owner questions. Do not skip the questions.
2. **Run a brief.** Use `system-prompt.md` as the job description (prompt) and their binder as the context. Follow `skills/four-whys-research/SKILL.md` for the research method.
3. **Score it.** This is the ride along (evals): `python3 evals/check_brief.py <file>` checks 34 rules. Do not argue with it, fix the brief.
...
## Conventions
- `skills/*/SKILL.md` are playbooks (skills), procedures written in plain markdown. Claude Code loads them automatically. If your runtime does not, read the relevant one before the step it covers.
- `example-pack/` is a finished binder for a real company, built only from public pages. Use it as the shape to copy, never as the person's own facts.
...
- The original ride along cases, which named real people who never agreed to be examples. Help the person build their own.
