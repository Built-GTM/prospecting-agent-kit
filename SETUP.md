...
1. **Give Claude your company's website.** Claude runs the `build-context-pack` playbook. It reads your site and drafts six files: `company.md`, `icp.md`, `personas/`, `problems/`, `signals.md`, and `proof.md`. Every field is marked as found (with a link), inferred (Claude's guess), or yours to answer.
...
4. **Approve the binder.** Read it, and reply **"approve the pack"** or say what to change. Only an approved binder reaches the agent. Reps never edit it directly. Changes come back through you.
