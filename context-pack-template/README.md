...
**The onboarding binder (context pack).** You are not writing software here. You are onboarding a rep, and this folder is the binder you hand them on day one.

Without it the agent invents things about your business on a live call: a customer you never signed, a result nobody can source, a confident pitch to a company that already pays you. It has no way to know better, because nothing here told it.

The knowledge half of the agent. The agent is the method: how to research, the honesty rules, the shape of the output. The binder is what your team knows: who you sell to, what you solve, when to reach out, and what proves it.

**One agent, many binders.** The method never mentions your company, so the day you change what you sell, or who you sell to, you edit six files and nothing else.
...
6. One owner approves changes before they go live. Reps never edit the running binder.
...
That is a working binder. Everything past it makes the briefs better, not possible.

## Before you rename a section
Every file here uses the same headings as `example-pack/`, which is a finished binder that passed the ride along (evals). Several of those headings are named directly in `system-prompt.md` and in `skills/four-whys-research/SKILL.md`. Renaming one does not break loudly, it just quietly switches off the rule that reads it. If you need a different shape, change the agent's rules in the same commit.
